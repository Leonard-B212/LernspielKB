...
document.getElementById("class-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const teacherID = Number(
      document.getElementById("class-teacher-id").value
    );
...
document.getElementById("student-form").addEventListener("submit", async (event) => {
  ...
  try {
    const classID = Number(
      document.getElementById("student-class-id").value
    );
...
async function loadClasses() {
  ...
  try {
    ...
    classes.forEach((schoolClass) => {
      ...
      option.textContent =
        `${schoolClass.className} (ID ${schoolClass.classID})`;
    ...
    showMessage(
      message,
      classes.length ? "" : "Noch keine Klassen vorhanden."
    );
  } catch (error) {
    showMessage(message, error.message, true);
  }
...
async function loadUsers() {
  ...
  try {
    ...
    showMessage(
      message,
      users.length ? "" : "Noch keine Benutzer vorhanden."
    );
  } catch (error) {
    showMessage(message, error.message, true);
  }
