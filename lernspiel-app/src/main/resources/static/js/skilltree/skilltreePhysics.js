...
export function initializeSkilltreePhysics({
    ...
    const nodeElements = [
        ...skilltreeElement.querySelectorAll(".skilltree-level-node")
    ];

    const categoryElements = [
        ...skilltreeElement.querySelectorAll(".skilltree-category-node")
    ];
    ...
    const nodes = nodeElements.map(createPhysicsNode);
    const startTime = performance.now();

    // Berechnet und rendert einen einzelnen Simulationsschritt.
    function animate(currentTime) {
        const elapsed = currentTime - startTime;
        const progress = Math.min(elapsed / SIMULATION_DURATION, 1);

        // Startet bei 1 und nähert sich während der Simulation 0.
        const intensity = Math.pow(1 - progress, 1.5);

        applyRepulsion(nodes, intensity);
        applyCategoryRepulsion(nodes, categoryElements, intensity);
        updateNodes(nodes);
...
function createPhysicsNode(element) {
    const baseX = Number.parseFloat(
        element.style.getPropertyValue("--node-x")
    ) || 0;

    const baseY = Number.parseFloat(
        element.style.getPropertyValue("--node-y")
    ) || 0;

    // Kleine zufällige Startabweichungen erzeugen das kurze Einpendeln beim Laden.
...
// Berechnet die gegenseitige Abstoßung benachbarter Level-Nodes.
function applyRepulsion(nodes, intensity) {
    for (let firstIndex = 0; firstIndex < nodes.length; firstIndex++) {
        for (let secondIndex = firstIndex + 1; secondIndex < nodes.length; secondIndex++) {
            ...
            first.velocityX -= normalizedX * force;
            first.velocityY -= normalizedY * force;

            second.velocityX += normalizedX * force;
            second.velocityY += normalizedY * force;
        }
    }
}

// Hält Level-Nodes auf Abstand zu den festen Kategorie-Nodes.
function applyCategoryRepulsion(nodes, categoryElements, intensity) {
    ...
    nodes.forEach((node) => {
        ...
        categoryElements.forEach((categoryElement) => {
            ...
            // Bestimmt den nächstgelegenen Punkt innerhalb der Kategorie-Box.
            const closestX = Math.max(
                categoryRect.left,
                Math.min(nodeCenterX, categoryRect.right)
            );

            const closestY = Math.max(
                categoryRect.top,
                Math.min(nodeCenterY, categoryRect.bottom)
            );

            const deltaX = nodeCenterX - closestX;
            const deltaY = nodeCenterY - closestY;
            const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
            ...
            // Drückt einen vollständig innerhalb der Kategorie liegenden Node nach unten heraus.
            if (distance === 0) {
                node.velocityY += STRENGTH * SAFE_DISTANCE * intensity;
                return;
            }

            const force = (SAFE_DISTANCE - distance) * STRENGTH * intensity;
